...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_bHiddenColUnhide    = 0;//TRUE;       // 0-width columns can be expanded via mouse
    m_bHiddenRowUnhide    = 0;//TRUE;       // 0-Height rows can be expanded via mouse

    m_bAllowColHide       = 0;//TRUE;       // Columns can be contracted to 0-width via mouse
    m_bAllowRowHide       = 0;//TRUE;       // Rows can be contracted to 0-height via mouse
    ...
    m_nResizeCaptureRange = 5;          // When resizing columns/row, the cursor has to be
                                        // within +/-3 pixels of the dividing line for
                                        // resizing to be possible
    m_pImageList          = NULL;       // Images in the grid
    m_bAllowDragAndDrop   = TRUE;      // for drag and drop - EFW - off by default
    ...
    SetPrintMarginInfo(3, 2, 7, 4, 1, 1, 1);
#endif
...
/*	VERIFY(m_pFont->CreateFont(
	   ...
	   _T("Arial ( ириллический)")));     // lpszFacename
*/
    // Create the printer font
    int nFontSize =14;
//    CString strFontName = "Arial ( ириллический)";
    CString strFontName = "Arial";
    m_pFont->CreateFont(nFontSize, 0,0,0, FW_NORMAL, 0,0,0, RUSSIAN_CHARSET,
                             OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, DEFAULT_QUALITY,
                             DEFAULT_PITCH | FF_DONTCARE, strFontName);


	m_bAutoExpandColumnsToFit=0;
}
...
CGridCtrl::~CGridCtrl()
{
    ...
    if (::IsWindow(m_TitleFind.GetSafeHwnd())) 
        m_TitleFind.DestroyWindow();

    DestroyWindow();

	delete m_pFont;

//≈сли это отладка, то при закрытии программы происходит ошибка (почему-то)
//поэтому - просто отключаем кусок кода :-(
#ifndef _DEBUG
#if !defined(GRIDCONTROL_NO_DRAGDROP) || !defined(GRIDCONTROL_NO_CLIPBOARD)
    // BUG FIX - EFW
...
#endif
	m_TitleFind.SetParentWnd(this);
    
    if (::IsWindow(m_hWnd))
        ModifyStyleEx(0, WS_EX_CLIENTEDGE, 0);
   
    bInProcedure = FALSE;
	//m_IntelliMouse.Attach(this);
    return TRUE;
}
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	...
	SetFont(m_pFont);
//	OnSetFont((WPARAM)m_pFont->GetSafeHandle(),0);
...
//{{AFX_MSG_MAP(CGridCtrl)
	...
	ON_NOTIFY(WM_ONSELECT, IDC_INPLACE_CONTROL,OnChoose)
	...
	ON_MESSAGE(WM_ONEDITCELL,OnWMEditCell)
...
void CGridCtrl::OnPaint()
{
	...
	try
	{
		...
		{// Draw raw - this helps in debugging vis problems.
			OnDraw(&dc);
		}
	}
...
void CGridCtrl::OnTimer(UINT nIDEvent)
{
	...
	int _m_MouseMode=m_MouseMode;//сохран€ем режим,т.к. оно может в процедуре OnKeyDown "испортитьс€"
    pt = origPt;
    if (pt.y > rect.bottom)
    {
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if(IsItemEditing(m_idCurrentCell.row,m_idCurrentCell.col))
    {
        CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
        return;
    }
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if (nChar == VK_DELETE)
    {
		int curRow=m_idCurrentCell.row;
		int curCol=m_idCurrentCell.col;
...
*/
		...
		if(!m_NotUseMyExtEdit&&m_bEditable)//Yuriy Iwanow 2001
		{
			DeleteRow(curRow);
			Refresh();

			if(curRow==curCount-1)
				nChar = VK_UP;
		}
		else
		{
...
*/
    ...
    if ((!m_NotUseMyExtEdit)&&nChar == VK_INSERT&&(!IsCTRLpressed())&&(!IsSHIFTpressed()))
    {
		if(m_bEditable)
		{
			int curCount=GetRowCount();
			if(m_idCurrentCell.row==curCount-1)
				SetRowCount(curCount+1);
			else
				InsertRow(NULL,m_idCurrentCell.row+1); 
			Refresh();
			...
			{
				if( GetRowHeight( next.row) > 0 &&!GetItemCoveredCell(next.row,next.col))
				{
					bFoundVisible = TRUE;
					break;
				}
			...
			OnEditCell(next.row, m_idCurrentCell.col, CPoint( -1, -1), VK_LBUTTON);
		}
    ...
    else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()) )
    {
        ...
        {
            if( GetColumnWidth( next.col) > 0 && !GetItemCoveredCell(next.row,next.col))
            {
                bFoundVisible = TRUE;
                break;
            }
    ...
    else if (nChar == VK_F2)
    {
		...
		InvalidateCellRect(m_idCurrentCell);
    }
    else
	if (nChar != VK_DELETE)
    {
    ...
    if (next != m_idCurrentCell)
    {
        ...
        //if (m_MouseMode == MOUSE_NOTHING)//а убрал проверку € :-)
        {
...
//ищем значение €чейки, начинающиес€ со строки str в текущей колонке
extern int afxEventMessage;
extern CStatusBar *pStatusBar;
int CGridCtrl::FindRowByStr(CString Str)
{
	int nStartRow=0;
	if(::IsWindow(m_TitleFind.m_hWnd))
	if(m_TitleFind.IsWindowVisible())
		nStartRow=m_LastFind_Row;

	CString Str1=Str,Str2=Str;
	Str1.MakeLower();
	Str2.MakeUpper();
	int lenfind=Str.GetLength();
	for(int nRow=nStartRow;nRow<GetRowCount();nRow++)
	{
		if(EscapePress())
		if(GetActiveWindow())
		{
		...
		{
			CString Str;
			Str.Format("»дет поиск %d. ƒл€ отмены нажмите ESC!",nRow);
			SetStatus(Str);
			m_TitleFind.Hide();
		}
		...
		CString temp1=GetItemText(nRow,m_idCurrentCell.col);
		CString temp2=temp1;
		temp1.MakeLower();
		temp2.MakeUpper();
		if(lenfind<=temp1.GetLength())
			if(temp1.Left(lenfind)==Str1)
			{
				m_LastFind_Row=nRow;
				return nRow;
			}
			if(temp2.Left(lenfind)==Str2)
			{
				m_LastFind_Row=nRow;
				return nRow;
			}
...
{
	if(m_FastFind)//Yuriy Iwanow 2002
	if(m_idCurrentCell.col>=0)
	if(m_idCurrentCell.col<GetColumnCount())
	{
		CColumnDescription &Column=aColumns[m_idCurrentCell.col];

		if(!m_bEditable || !Column.bEnable)
		if (!IsCTRLpressed())
		if(nChar>=' ')
		{
			CString Str;
			if (nChar != 8)
				Str.Format("%s%c",m_TitleFind.csFind,nChar);
			else
				Str=m_TitleFind.csFind.Left(m_TitleFind.csFind.GetLength()-1);

			afxEventMessage=1;
			SetStatus("»дет поиск. ƒл€ отмены нажмите ESC!");
			int nRow=FindRowByStr(Str);
			afxEventMessage=0;
			SetStatus("");
			if(nRow>=0)
			{
				m_TitleFind.Hide();
				m_TitleFind.csFind=Str;
				if(!Str.IsEmpty())
				{
					//SetFocusCell();
					CCellID cell;
					...
					ResetSelectedRange();
					EnsureVisible(nRow, m_idCurrentCell.col);

					SetFocusCell(cell);
					//SetFocusCell(cell);

					CRect TextRect;
					GetCellRect( m_idCurrentCell.row, m_idCurrentCell.col, &TextRect);
					m_TitleFind.Show(TextRect, Str);
				}
			}
		}
	}
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    OnEndEditCell(pgvItem->row, pgvItem->col, pgvDispInfo->Val);
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	if(m_bAutoExpandColumnsToFit)
	{
		m_bAllowDraw=0;
		ExpandColumnsToFit(0);   // This will remove any existing horz scrollbar
		m_bAllowDraw=1;
	}
	...
	CGridCellBase* pCellDefault=GetDefaultCell(0,0);
	CGridCellBase* pFixCellDefault=GetDefaultCell(0,1);
	if(pCellDefault)
		pCellDefault->SetCovered(0,0);
	if(pFixCellDefault)
		pFixCellDefault->SetCovered(0,0);
	//if(GetListMode())
	if(GetRowCount()>0)
	{
		CRect Rect;
		GetClientRect(Rect);
		int nHeight=GetRowHeight(GetRowCount()-1);
		if(nHeight>0)
		{
			int nDelta=(Rect.bottom-VisRect.bottom)/nHeight+1;
			if(nDelta>0)
			{
				maxVisibleRow+=nDelta;
			}
		}
		//maxVisibleRow
	}
    ...
    // draw all rect
	int nHeight=GetRowHeight(0);
    rect.bottom = -1;
    for (row = 0; row <= maxVisibleRow; row++)
    {
		if(row>=GetFixedRowCount())//сначала рисуем фиксированные строки
			row=max(row,minVisibleRow);//потом обычные

		if(HasContext())
			if(GetContext().FindMethod("ќбработка—обыти€")>=0)
			GetContext().CallFunction("ќбработка—обыти€",&CValue("ќтображение"), &CValue(row),&CValue(0),&CValue(),&CValue(),&CValue(),&CValue(),&CValue());


		if(row<GetRowCount())
			nHeight=GetRowHeight(row);
        ...
        {
			if(col>=GetFixedColumnCount())//сначала рисуем фиксированные колонки
				col=max(col,minVisibleCol);//потом обычные
			...
			else
			{
				...
				{
					if (pFixCellDefault)
						pFixCellDefault->Draw(pDC, row, col, rect, FALSE);
				}
				else
				{
					if (pCellDefault)
						pCellDefault->Draw(pDC, row, col, rect, FALSE);
				}
			}
        }
    }
...
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
    ...
    if (IsValid(m_idCurrentCell))
    {
		...
		CCellID id=GetTopleftNonFixedCell();
		if(m_idCurrentCell.row<id.row)//<=
		if (m_bListMode)
			SendMessage( WM_VSCROLL, SB_LINEUP, 0 );
        ...
        nMinRow <= nMaxRow && nMinCol <= nMaxCol)
    {
        for (int row = nMinRow; row <= nMaxRow; row++)
            for (int col = nMinCol; col <= nMaxCol; col++)
            {
				int nRow=row;
				int nCol=col;
				//поддержка объединенных €чеек
				CGridCellBase *pCell = GetItemCoveredCell(nRow, nCol);
				if (pCell)
				{
					pCell->GetCoveredRowCol(nRow, nCol);
				}

                int nState = GetItemState(nRow, nCol);
                ...
                if (bSelectCells)
                    SetItemState(nRow, nCol, nState | GVIS_SELECTED);
                else
                    SetItemState(nRow, nCol, GetItemState(nRow, nCol) & ~GVIS_SELECTED);

				if ( VisCellRange.IsValid() && VisCellRange.InRange(nRow, nCol) )
				{
	                // Redraw (immediately or at leisure)
	                if (bForceRepaint && pDC)
                              ...
                              BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	//поддержка объединенных €чеек
	CGridCellBase *pCell = GetCell(currentCell.row, currentCell.col);
	int nColCount=pCell->GetCoveredCol();
	if(nColCount>0)
	{
          SetSelectedRange(GetFixedRowCount(),
                         min(m_SelectionStartCell.col, currentCell.col),
                         GetRowCount()-1,
                         max(m_SelectionStartCell.col, currentCell.col+nColCount-1),
                           ...
                           BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	//поддержка объединенных €чеек
	CGridCellBase *pCell = GetCell(currentCell.row, currentCell.col);
	int nRowCount=pCell->GetCoveredRow();
	//int nColCount=pCell->GetCoveredCol();
	if(nRowCount>0)
	{
        SetSelectedRange(min(m_SelectionStartCell.row, currentCell.row),
                         GetFixedColumnCount(),
                         max(m_SelectionStartCell.row, currentCell.row+nRowCount-1),
                         GetColumnCount()-1,
                         bForceRedraw, bSelectCells);
		return;
	}
...
{
    SendMessageToParent(Selection.GetTopLeft().row, Selection.GetTopLeft().col, GVN_BEGINDELETEITEMS);  // pjp
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
	{
        for (int col = Selection.GetMinCol(); col <= Selection.GetMaxCol(); col++)
		{
            ...
            if ( m_arRowHeights[row] > 0 && m_arColWidths[col]  > 0)
			{
				...
				SetItemCell(row,col,0);
				SetUnCoveredCells(row,col);
			}
		}
	}
    SendMessageToParent(Selection.GetTopLeft().row, Selection.GetTopLeft().col, GVN_ENDDELETEITEMS); // pjp
    Refresh();
}
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
    ...
    if (abs(point.y - start.y) <= m_nResizeCaptureRange && idCurrentCell.row != 0) //||
...
BOOL CGridCtrl::MouseOverColumnResizeArea(CPoint& point)
{
	...
	{
		if(point.x > GetFixedColumnWidth()+m_nResizeCaptureRange)
			return FALSE;
	}
	...
	if(idCurrentCell.col<=0)
        return FALSE;

	if(!aColumns[idCurrentCell.col].bVisible)
	{
		point.y--;
		return MouseOverColumnResizeArea(point);
	}
    ...
    if (abs(point.x - start.x) <= m_nResizeCaptureRange || 
...
CCellID CGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
	...
	//поддержка объединенных €чеек
	CGridCellBase *p=GetItemCoveredCell(cellID.row,cellID.col);
	if(p)
	{
		p->GetCoveredRowCol(cellID.row,cellID.col);
	}
...
{
	if(::IsWindow(m_hWnd))
	if(m_SelectedCellMap.GetCount()>1)
		Invalidate();
    m_PrevSelectedCellMap.RemoveAll();
...
BOOL CGridCtrl::GetCellRect(int nRow, int nCol, LPRECT pRect,BOOL bCorrect)
{
    CPoint CellOrigin;
	...
	CGridCellBase *p=GetCell(nRow,nCol);
	if(p)
	if(p->IsCovered())//поддержка объединенных €чеек
	{
		int nCoveredRow=p->GetCoveredRow()-1;
		int nCoveredCol=p->GetCoveredCol()-1;
		for (int row=nRow+1; row <= nRow+nCoveredRow; row++)
			pRect->bottom+=GetRowHeight(row);
		for (int col=nCol+1;col <= nCol+nCoveredCol; col++)
			pRect->right+=GetColumnWidth(col);
...
//*
		...
		{
			CCellID idTopLeft = GetTopleftNonFixedCell();
			if (!(nCol == idTopLeft.col && nRow == idTopLeft.row))
			{
				CRect TopLeftRect;
				GetCellRect(idTopLeft.row,idTopLeft.col,TopLeftRect);
				pRect->top=max(pRect->top,TopLeftRect.top);
				pRect->left=max(pRect->left,TopLeftRect.left);
			}
		}
//*/
    ...
    TRY 
    {
        ...
        if (addedCols > 0)
        {
            ...
            for (int col = startCol; col < nCols; col++)
			{
				...
				aColumns[col].nColumnWidth=m_arColWidths[col];
			}
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    TRY
    {
        if (nColumn < 0)
        {
            nColumn = m_nCols;
            m_arColWidths.Add(0);
			aColumns.Add(CColumnDescription());
            if (!GetVirtualMode())
            {
                for (int row = 0; row < m_nRows; row++)
                {
        ...
        else
        {
			...
			aColumns.InsertAt(nColumn, CColumnDescription());
            if (!GetVirtualMode())
            {
				CGridCellBase* pCoveredPrev=0;
                for (int row = 0; row < m_nRows; row++) 
                {
                    GRID_ROW* pRow = m_RowData[row];
                    if (!pRow)
                        return -1;
					CGridCellBase* pCell=CreateCell(row, nColumn);
                    pRow->InsertAt(nColumn, pCell);
                }
				//контроль объединенных €чеек
                for (row = 0; row < m_nRows; row++) 
                {
                    GRID_ROW* pRow = m_RowData[row];
                    if (!pRow)
                        return -1;
					CGridCellBase* pCell0=pRow->GetAt(nColumn);
					if(pCell0)
					if(pCell0->GetCoveredRow())
					{
						SetCoveredCells(row,nColumn,pCell0->GetCoveredRow(),pCell0->GetCoveredCol());
					}
    ...
    m_arColWidths[nColumn] = GetDefCellWidth();//GetTextExtent(0, nColumn, strHeading).cx;
	aColumns[nColumn].nColumnWidth=m_arColWidths[nColumn];
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    if (!GetVirtualMode())
    {
        ...
        for (int col = 0; col < m_nCols; col++)
        {
			...
			CGridCellBase* pCell=CreateCell(nRow, col);
        ...
        for ( col = 0; col < m_nCols; col++)
        {
			...
			if(nRow>0)
			{
				CGridCellBase* pCell0=pRow->GetAt(col);
				if(pCell0)
				if(pCell0->GetCoveredRow())
				{
					SetCoveredCells(nRow,col,pCell0->GetCoveredRow(),pCell0->GetCoveredCol());
				}
			}
        }
   }
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
	...
	//if(aColumns[nCol].m_pRtcDefault)
	//	pRtcDefault=aColumns[nCol].m_pRtcDefault;

    if (!pRtcDefault || !pRtcDefault->IsDerivedFrom(RUNTIME_CLASS(CGridCellBase)))
    {
        ASSERT( FALSE);
        return NULL;
    }
    CGridCellBase* pCell = (CGridCellBase*) pRtcDefault->CreateObject();
    if (!pCell)
        return NULL;
...
BOOL CGridCtrl::DeleteColumn(int nColumn)
{
    ...
    if (!GetVirtualMode())
    {
        ...
        for (int row = 0; row < GetRowCount(); row++)
        {
			...
			CGridCellBase* pCell=pRow->GetAt(nColumn);
			if(pCell)
			if(CGridCellBase* pCovered=pCell->GetCoveredCell())
			{
				if(pCoveredPrev!=pCovered)
					pCovered->SetCovered(pCovered->GetCoveredRow(),pCovered->GetCoveredCol()-1);
				pCoveredPrev=pCovered;
			}
...
BOOL CGridCtrl::DeleteRow(int nRow)
{
    ...
    if (!GetVirtualMode())
    {
        ...
        for (int col = 0; col < GetColumnCount(); col++)
		{
			CGridCellBase* pCell=pRow->GetAt(col);
			if(pCell)
			if(CGridCellBase* pCovered=pCell->GetCoveredCell())
			{
				//TODO: здесь Ѕј√...
				if(pCoveredPrev!=pCovered)
					pCovered->SetCovered(pCovered->GetCoveredRow()-1,pCovered->GetCoveredCol());
				pCoveredPrev=pCovered;
			}
...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int nType=pCell1->GetValue().GetType();
	if(nType==TYPE_NUMBER||nType==TYPE_DATE)
		return pfnCellNumericCompare(lParam1, lParam2, lParamSort);

	return _tcscmp(pCell1->GetText(), pCell2->GetText());
/*	CValue Val1=pCell1->GetValue();
	CValue Val2=pCell1->GetValue();
...
int CALLBACK CGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	if(nType==TYPE_DATE)
	{
		nValue1 = pCell1->GetValue().GetDate();
		nValue2 = pCell2->GetValue().GetDate();
	}
	else
	{
		nValue1 = atof(pCell1->GetText());
		nValue2 = atof(pCell2->GetText());
	}
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
	...
	pCell->SetCovered(pItem->nCoveredRow,pItem->nCoveredCol);
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
	...
	pItem->nCoveredRow=pCell->GetCoveredRow();
	pItem->nCoveredCol=pCell->GetCoveredCol();
...
BOOL CGridCtrl::SetItemValue(int nRow, int nCol, CValue &Val)
{
    if (GetVirtualMode())
	{
		if(HasContext())
			if(GetContext().FindMethod("ќбработка—обыти€")>=0)
				GetContext().CallFunction("ќбработка—обыти€",&CValue("«начение"), &CValue(nRow),&CValue(nCol),&Val,&CValue(),&CValue(),&CValue(),&CValue());
        return FALSE;
	}
	...
	CString csType=aColumns[nCol].csType;
	if(!csType.IsEmpty())
	{
		CString csType2=Val.GetTypeString();
		if(Val.GetType()>TYPE_ARRAY)
		{
			CString csKind=Val.CallFunction("¬ид");
			if(!csKind.IsEmpty())
			{
				csType2=csType2+"."+csKind;
			}
		}
		CValue Val2=Val;
		if(mUpper(csType)==mUpper(csType2))
		{
		    pCell->SetValue(Val2);
		}
		else
		{
			GetItemValue(nRow,nCol);
		    pCell->SetData(Val2);
		}
	}
	else
...
BOOL CGridCtrl::SetLeftSpace(int nRow, int nCol, int nCount)
{
    if (GetVirtualMode())
        return FALSE;
...
//    pCell->SetLeftSpace(nCount);
    SetModified(TRUE, nRow, nCol);
    return TRUE;
}

/*
inline void CGridCtrl::ClearItem(int nRow, int nCol)
{
    if (!GetVirtualMode())
	{
	    CGridCellBase* pCell = GetCell(nRow, nCol);
...
BOOL CGridCtrl::SetColumnWidth(int nCol, int width)
{
	...
	aColumns[nCol].nColumnWidth=width;
    ResetScrollBars();
...
void CGridCtrl::SetColumnVisible(int nCol,BOOL bSet)
{
	int width=aColumns[nCol].nColumnWidth;
	if(bSet)
	{
	...
	aColumns[nCol].bVisible=bSet?1:0;
}
BOOL CGridCtrl::GetColumnVisible(int nCol)
{
	return aColumns[nCol].bVisible;
}
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	...
	//восстанавливаем оригинальные значени€
    for (i = nFirstColumn; i < GetColumnCount(); i++)
	{
		if(aColumns[i].bVisible)
			m_arColWidths[i]=aColumns[i].nColumnWidth;
		else
			m_arColWidths[i]=0;
	}
...
/*    
	for (i = nFirstColumn; i < GetColumnCount(); i++)
	if(!aColumns[i].bVisible)//невидимые колонки не учитываем
        nNumColumnsAffected -= 1;
	if(nNumColumnsAffected<=0)
		return;
*/
    long virtualWidth = GetVirtualWidth();

	//вычитаем ширину фиксированных колонок
	for (i = 0; i < nFirstColumn; i++)
	{
		if(aColumns[i].bVisible)
		{
			rect.right-=m_arColWidths[i];
			virtualWidth-=m_arColWidths[i];
		}
	}
	...
	if(nDifference>0 && virtualWidth)
	{
	    EnableScrollBars(SB_HORZ, FALSE);

		int nSum=0;
		double KK=double(rect.Width())/ double((int) virtualWidth);
		for (i = nFirstColumn; i < GetColumnCount(); i++)
			if(aColumns[i].bVisible)
			{
				m_arColWidths[i] = double(m_arColWidths[i])*KK;
				nSum+=m_arColWidths[i];
			}

		//ошибку округлени€ записываем в самую последнюю видимую колонку
/*		int nDelta=(int) virtualWidth-nSum;
		if(nDelta>0)
		for (i = GetColumnCount()-1; i >= nFirstColumn; i--)
		if(aColumns[i].bVisible)
		{
			m_arColWidths[i]+=nDelta-2;
			break;
		}
...
#define DELTA_FAST_POS	30
// EnsureVisible supplied by Roelf Werkman + ёрий »ванов
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
...

	{
		...
		//*
		// set the scroll to the approximate position of row (Nigel Page-Jones + Yu. Iwanow)
		SCROLLINFO scrollInfo;
		if(down>=DELTA_FAST_POS||up>=DELTA_FAST_POS)//только если особо требуетс€
		{
			GetScrollInfo(SB_VERT, &scrollInfo);
			scrollInfo.nPos = round((double)scrollInfo.nMax * (double)nRow / (double)GetRowCount());
			SetScrollInfo(SB_VERT, &scrollInfo, TRUE);
		}

		if(right>=DELTA_FAST_POS||left>=DELTA_FAST_POS)//только если особо требуетс€
		{
			GetScrollInfo(SB_HORZ, &scrollInfo);
			scrollInfo.nPos = round((double)scrollInfo.nMax * (double)nCol / (double)GetColumnCount());
...
void CGridCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    //m_bLMouseButtonDown = ((nFlags & MK_LBUTTON) == MK_LBUTTON);

//	OutToDebug("ButtonDown = %d, m_MouseMode=%d\n",m_bLMouseButtonDown ,m_MouseMode);
        ...
        case MOUSE_SELECT_CELLS:
            {
                ...
                if (idCurrentCell != GetFocusCell())
                {
					...
					if(idCurrentCell.row != -1 && idCurrentCell.row < GetFixedRowCount())
					{
						CCellID id=GetTopleftNonFixedCell();
						idCurrentCell.row=id.row;
					}
					if(idCurrentCell.col != -1 && idCurrentCell.col < GetFixedColumnCount())
					{
						CCellID id=GetTopleftNonFixedCell();
						idCurrentCell.col=id.col;
					}
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	CRect Rect;
	GetCellRect(m_LeftClickDownCell.row,m_LeftClickDownCell.col,Rect);
	InvalidateRect(Rect);
	...
	//EnsureVisible(m_LeftClickDownCell.row,m_LeftClickDownCell.col);
...
/*
            ...
            if( bLookForVisible)
            {
                ...
                while( m_LeftClickDownCell.col - iOffset >= 0)
                {
					...
					if( aColumns[m_LeftClickDownCell.col - iOffset].bVisible)
                    {
                        bFoundVisible = TRUE;
                        break;
                    }
...
#endif
    else if (m_MouseMode == MOUSE_SIZING_COL && IsEditable())
    {
        CRect rect;
        GetClientRect(rect);
        ...
        {   
		    ...
		    SetModified(TRUE, m_LeftClickDownCell.row, m_LeftClickDownCell.col);
			m_bAutoExpandColumnsToFit=0;//запрещаем автоизменение размера
        }
    }
    else if (m_MouseMode == MOUSE_SIZING_ROW && IsEditable())
    {
        CRect rect;
        GetClientRect(rect);
...
//		SetCurrentCell(FocusCell.row, FocusCell.col);

//		SetFocusCell(FocusCell.row, FocusCell.col);
		EnsureVisible(FocusCell.row, FocusCell.col); // Make sure cell is visible


//		SetCurrentCell(FocusCell.row,FocusCell.col);
/*		SetFocusCell(FocusCell);
		if (GetListMode())
			SelectRows(FocusCell, TRUE, FALSE);
		else
			SelectCells(FocusCell, TRUE, FALSE);
*/
...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
	...
	if(!IsExtMode)
	{
		...
		m_nLastRow=GetRowCount()-1;
		m_nLastColumn=GetColumnCount()-1;
		i=1;
	}
    for (; i < m_nLastColumn+1; i++)
        lVirtualWidth += m_arColWidths[i];
    ...
    m_CharSize = pDC->GetTextExtent(_T("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSATUVWXYZабвгдеЄжзийклмопрстуфхччщэю€ьъјЅ¬√ƒ≈®∆«»… Ћћќѕ–—“”‘’„„ўЁёя№Џ"),52+60);
    m_CharSize.cx = m_CharSize.cx/(52+60);
    ...
    {
        m_LogicalPageSize.cx = ScreenPixelsPerInch.cx * m_PaperSize.cx / PaperPixelsPerInch.cx * 100 / 97;
        m_LogicalPageSize.cy = ScreenPixelsPerInch.cy * m_PaperSize.cy / PaperPixelsPerInch.cy * 100 / 99;
    }
    else
    {
        m_LogicalPageSize.cx = ScreenPixelsPerInch.cx * m_PaperSize.cx / PaperPixelsPerInch.cx;// * 3 / 4;
        m_LogicalPageSize.cx = max(m_LogicalPageSize.cx,lVirtualWidth+nMargins);
    ...
    m_nPageHeight = m_LogicalPageSize.cy - nFixedRowHeight
    ...
    m_nNumPages = 1;
	if(!IsExtMode)
    for (int row = GetFixedRowCount(); row < m_nLastRow+1; row++)
    {
    ...
    if( m_bWysiwygPrinting)
    {
        ...
        for( i1 = GetFixedColumnCount(); i1 < m_nLastColumn+1; i1++)
        {
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    pDC->OffsetWindowOrg(0, -nFixedRowHeight);
    ...
    while(  m_nCurrPrintRow < m_nLastRow+1
	...
	{
		for( i2 = m_nPrintColumn; i2 < m_nLastColumn+1; i2++)
		{
	...
	if(!IsExtMode)
	{
		...
		pInfo->m_rectDraw.bottom = nFixedRowHeight;
		if( m_nPrintColumn == GetFixedColumnCount())
		{
			...
			pDC->OffsetWindowOrg( -iColumnOffset, -nFixedRowHeight);
		}
		else
		{
    ...
    if (m_nCurrPrintRow >= m_nLastRow+1) 
		return;
    ...
    while (m_nCurrPrintRow < m_nLastRow+1)
    {
        ...
        for (int col = m_nPrintColumn; col < m_nLastColumn+1; col++)
        {
            rect.left = rect.right+1;
            ...
            if (pCell)
                ...
                pCell->Draw(pDC, m_nCurrPrintRow, col, rect);
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
    ...
    if (GetHeaderSort())
    {
		...
		{
			if(pfnCellTextCompare==m_pfnCompare)
				SortItems(cell.col, TRUE);
			else
				SortItems(cell.col, FALSE);
		}
        Invalidate();
    }
...
void CGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
    ...
    if (SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT) >= 0)
    {
        ...
        if (pCell)
		{
			...
			{
				CValue vFlag=1;
				CallFunction("ѕриЌачале¬ыбора«начени€",String(aColumns[nCol].csName),vFlag,CValue(0));
				CallFunction("OnStartValueChoice",String(aColumns[nCol].csName),vFlag,CValue(0));
				if(!vFlag.GetNumber())
					return;
			}
...
void CGridCtrl::CallFunction(CString csFunction,CValue &vParam1,CValue &vParam2,CValue &vParam3)
{
	if(!pRunContext)
		return;
	if(!pRunContext->pProcUnit)
		return;
	try
	{
		pRunContext->pProcUnit->CallFunction(csFunction,vParam1,vParam2,vParam3);
	}
	catch(CTranslateError *)
	{
		DoError();
...
// virtual
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CValue &Val)
{
	BOOL bEscape=((GetKeyState(VK_ESCAPE) & (1 << (sizeof(SHORT)*8-1))) != 0 );
	...
	{
		CValue vFlag=1;
		CallFunction("ќбработка¬ыбора«начени€",Val,String(aColumns[nCol].csName),vFlag);
		CallFunction("ProcessValueChoice",Val,String(aColumns[nCol].csName),vFlag);
		if(!vFlag.GetNumber())
			bEscape=1;
	}
	...
	if(!bEscape)
	{
		...
		CValue currentValue=GetItemValue(nRow, nCol);
		SetItemValue(nRow, nCol, Val);
		cEditValue=Val;
		if (ValidateEdit(nRow, nCol, Val.GetString()) && 
			SendMessageToParent(nRow, nCol, GVN_ENDLABELEDIT) >= 0)
		{
			SetModified(TRUE, nRow, nCol);
			RedrawRow(nRow);//перерисовываем всю строку, т.к. возможно там есть вычисл€емые формулы
			//Message("End");
		}
		else
		{
			SetItemValue(nRow, nCol, currentValue);
		}
		cEditValue.Reset();
	}
	else
...
//ѕолучени€ значени€ €чейки с проверкой на соответствие типа колонки
CValue CGridCtrl::GetItemValue(int nRow, int nCol) const
{
    if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols)
        return CValue();
...
//	return pCell->GetValue();
	...
	CValue &Ret=pCell->GetValue();
	if(!aColumns[nCol].csType.IsEmpty())
	{
		if(Ret.GetType()==0)
			Ret.CreateObject(aColumns[nCol].csType);
			//pCell->SetValue(GetTypeValue(aColumns[nCol].csType));
		//else
		//	Ret=pCell->GetValue();
	}
...
void CGridCtrl::PrintOther(CDC *pDC, CPrintInfo *pInfo)
{

}

void CGridCtrl::SetCurrentCell(int i,int j) //Yuriy Iwanow 2003
{
	CPoint p(1,1);
	CCellID cell(i,j);
    ...
    SetFocusCell(cell);
	EnsureVisible(i,j); // Make sure cell is visible

/*	if (GetListMode())
		SelectRows(cell, TRUE, FALSE);
	else
		SelectCells(cell, TRUE, FALSE);
*/
...
void CGridCtrl::OnChoose(NMHDR* pNMHDR, LRESULT* pResult)
{
    *pResult = 0;
};
...
BOOL CGridCtrl::DoEditUndo(UINT /*nID*/)
{
    ...
    // If the cell is valid and is being edited, then call its edit window undo function.
...
void CGridCtrl::SetCoveredCells()
{
	CCellRange Selection=GetSelectedCellRange();
	SetCoveredCells(
		Selection.GetMinRow(),
		Selection.GetMinCol(),
		Selection.GetMaxRow()-Selection.GetMinRow()+1,
		Selection.GetMaxCol()-Selection.GetMinCol()+1);
	SetCurrentCell(Selection.GetMinRow(),Selection.GetMinCol());
	Invalidate();
}
void CGridCtrl::SetUnCoveredCells()
{
	CCellRange Selection=GetSelectedCellRange();
	SetUnCoveredCells(
		Selection.GetMinRow(),
...
void CGridCtrl::SetUnCoveredCells(int nStartRow,int nStartCol,int nSetModified)
{
	CGridCellBase *pCell = GetCell(nStartRow, nStartCol);
	if(!pCell)
	{
		ASSERT(FALSE);
		return;
	...
	{
		pCell->SetCovered(0,0);
		for(int nRow=nStartRow;nRow<nStartRow+nRowCount;nRow++)
			for(int nCol=nStartCol;nCol<nStartCol+nColCount;nCol++)
			{
				CGridCellBase *pCell = GetCell(nRow, nCol);
				if (pCell)
				{
		...
		if(nSetModified)
			SetModified(1,nStartRow+nRowCount-1,nStartCol+nColCount-1);
	}
}
void CGridCtrl::SetCoveredCells(int nStartRow,int nStartCol,int nRowCount,int nColCount)
{
	SetUnCoveredCells(nStartRow,nStartCol);
	if(nRowCount<=1&&nColCount<=1)
		return;
	SetHeaderSort(0);
	CGridCellBase *pParentCell=0;

	if(nStartRow+nRowCount>=GetRowCount())
		nRowCount=GetRowCount()-nStartRow;
	if(nStartCol+nColCount>=GetColumnCount())
		nColCount=GetColumnCount()-nStartCol;
	...
	for(int nRow=nStartRow;nRow<nStartRow+nRowCount;nRow++)
		for(int nCol=nStartCol;nCol<nStartCol+nColCount;nCol++)
		{
			...
			if (pCell)
			{
				...
				{
					pCell->SetCovered(nRowCount,nColCount);
					pParentCell=pCell;
				}
				else
				{
					pCell->SetCoveredCell(pParentCell);
				}
...
CGridCellBase* CGridCtrl::GetItemCoveredCell(int nRow, int nCol)const
{
    if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols)
        return 0;
    ...
    return pCell->GetCoveredCell();
}

void CGridCtrl::SetItemCell(int nRow, int nCol, class CCell* pSet)
{
    if (GetVirtualMode())
        return;
...
CCell* CGridCtrl::GetItemCell(int nRow, int nCol)
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    if (!pCell)
        return 0;

    return pCell->GetCell();
}
void CGridCtrl::SetLastRowCol(int nRow,int nCol,int nAdd)
{
	if(1==nAdd)
	{
		if(nRow>m_nLastRow)
...
BOOL CGridCtrl::HasContext()
{
	CCallBackInfo *pCallBack=(CCallBackInfo *)m_lParam;
	if(pCallBack)
	if(pCallBack->pControl)
	if(pCallBack->pControl->eventVal.nType!=TYPE_EMPTY)
		return 1;

	return 0;
}
CValue CGridCtrl::GetContext()
{
	CValue Ret;
	CCallBackInfo *pCallBack=(CCallBackInfo *)m_lParam;
	if(pCallBack)
	...
	if(pCallBack->pControl->eventVal.nType!=TYPE_EMPTY)
	{
		Ret=pCallBack->pControl->eventVal;
	}
	return Ret;
}
...
LRESULT CGridCtrl::OnWMEditCell(WPARAM wParam, LPARAM lParam)
{
	...
	int nCol=(int)lParam;
	CPoint point;
	GetCellOrigin(nRow,nCol,&point);
	OnEditCell(nRow,nCol,point,VK_LBUTTON);
