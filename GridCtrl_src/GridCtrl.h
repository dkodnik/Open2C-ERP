...
//#include "..\OX\OXIntelliMouse.h"

#define WM_ONEDITCELL WM_USER+17

#define GRIDCTRL_NO_UNDO
...
public:
	...
	CString csName;//Идентификатор
	CString csTitle;//Заголовок
	CString csFormule;//Формула
	CString csType;//Тип значения
	int		nLength;//Длина
	int		nPrec;//Точность
	bool bVisible;//Видимость
	bool bEnable;//Доступность для редактирования
	bool bTab;//Пропускать при вводе
	int	 nColumnWidth;//служебный параметр - устанавливается при работе с видимостью колонки
	CString csDescription;//Описание
};
...
public:
	...
	class CValueGrid *pGrid;
	CString csFunction;
	class CDynControl *pControl;
	CValue *pFormValue;
};
...
///////////////////////////////////////////////////////////////////////////////////
//Мои атрибуты и функции
///////////////////////////////////////////////////////////////////////////////////
	//АТРИБУТЫ:
    bool m_NotUseMyExtEdit;
	BOOL m_FastFind;
	void SetCurrentCell(int i,int j) ;
	int	 m_LastFind_Row;
    CTitleTip   m_TitleFind;             // Title tips for fast find

	CArray <CColumnDescription,CColumnDescription> aColumns;
	CGridCellBase* GetItemCoveredCell(int nRow, int nCol)const;

	BOOL IsExtMode;//режим таблицы 1С - номера строк и колонок нумеруются принудительно
	CFont *m_pFont;

	int m_nLastColumn;//последняя колонка в которой есть данные для печати на принтере
	int m_nLastRow;//тоже для строки

	CRunContext *pRunContext;//контекст интерпретации формул
	int nDrawRow;//текущая интерпретируемая строка и колонка
	int nDrawCol;

	CValue cEditValue;//редактируемое значение

	BOOL m_bAutoExpandColumnsToFit;

	//МЕТОДЫ:
	int FindRowByStr(CString Str);
	//поддержка объединенных ячеек
	void SetCoveredCells(int nRow,int nCol,int nRowCount,int nColCount);
	void SetUnCoveredCells(int nStartRow,int nStartCol,int nSetModified=1);
	void SetCoveredCells();
	void SetUnCoveredCells();
	void SetLastRowCol(int nRow,int nCol,int nAdd=1);
	int GetLastRow(){return m_nLastRow;};
	int GetLastCol(){return m_nLastColumn;};
...
public:
	...
	void SetColumnVisible(int nCol,BOOL bSet);
	BOOL GetColumnVisible(int nCol);

    BOOL GetCellOrigin(int nRow, int nCol, LPPOINT p);
    BOOL GetCellOrigin(const CCellID& cell, LPPOINT p);
    BOOL GetCellRect(int nRow, int nCol, LPRECT pRect,BOOL bCorrect=1);
...
public:
    ...
    void   SetItemCell(int nRow, int nCol, CCell*);
	
    BOOL   SetItemValue(int nRow, int nCol, CValue &Value);
    ...
    CValue  GetItemValue(int nRow, int nCol) const;
    CCell*	GetItemCell(int nRow, int nCol);
    ...
    //inline void ClearItem(int nRow, int nCol);

    BOOL   SetLeftSpace(int nRow, int nCol, int nCount);
...
// Implementation
public:
	virtual void PrintOther(CDC *pDC, CPrintInfo *pInfo);
    virtual ~CGridCtrl();
...
public:
    ...
    virtual void  OnEndEditCell(int nRow, int nCol, CValue &Val);
...
public:
    ...
    CArray <int,int> m_arRowHeights, m_arColWidths;
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        ...
        cell.SetValue(gvdi.item.Value);
        cell.SetGrid((CGridCtrl*)this);
		//cell.SetCovered(gvdi.item.nCoveredRow,gvdi.item.nCoveredCol);
		//cell.SetCell(gvdi.item.m_pCell);
